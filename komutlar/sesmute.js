...
let puan = await data.fetch(`cezapuan.${user.id}`)
...
**${user.user}** Adlı Kullanıcı Ses Kanallarından Uzaklaştırıldı. (\`#${muteid-(-1)}\`)
 ...
 let puanlogss = message.guild.channels.find(c => c.id == ayarlar.puanlog) 
 puanlogss.send(`${user}: aldığınız **#${muteid-(-1)}** ID'li ceza ile ${!puan ? 3 : puan} ceza puanına ulaştınız.`)
 data.add(`cezapuan.${user.id}`,3)

let log_channel = message.guild.channels.find('name', 'mute-log');
data.add('id',1)
...
`)
log_channel.send(embed)
require("quick.db").add(`${message.guild.id}.${user.id}.voiceMuteAmount`, 1)
}, ms(time));

//let logs1 = message.guild.channels.find(c => c.id == ayarlar.puanlog) 
//logs1.send(`${user}: aldığınız **#${muteid-(-1)}** ID'li ceza ile ${!puan ? 20 : puan} ceza puanına ulaştınız.`)
