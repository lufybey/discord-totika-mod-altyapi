...
exports.run = async (codeAcademy, message, args) => {
  ...
  let totika = message.guild.roles.find(a => a.name === "@everyone");
  if(message.channel.permissionsfor(totika).has('SEND_MESSAGES')) {
    message.channel.overwritePermissions(totika, {
      SEND_MESSAGES: false,
    });
