...
if(!kisi.voiceChannel) return message.channel.send(new Discord.RichEmbed().setColor(client.randomrenk()).setAuthor(message.author.tag , message.author.avatarURL).setDescription('Yanına gitmek istediğin kişi sesli kanalda değil!').setColor('black'))
  ...
  m.awaitReactions(filter, { max: 1, time: 60000, errors: ['time'] })
    .then(collected => {
        ...
        if (reaction.emoji.name === '✔️') {
            message.channel.send(new Discord.RichEmbed().setColor(client.randomrenk()).setAuthor(`${message.author.tag}`, message.author.avatarURL).setColor(client.randomrenk()).setDescription('<@'+message.author.id+'> isteğin kabul edildi. '));
      message.member.setVoiceChannel(kisi.voiceChannel.id)
        } else {
            message.channel.send(new Discord.RichEmbed().setColor(client.randomrenk()).setAuthor(`${message.author.tag}`, message.author.avatarURL).setColor(client.randomrenk()).setDescription('<@'+message.author.id+'> isteğin reddedildi. '))
        }
    })
    ))
  //✔️ ❌
