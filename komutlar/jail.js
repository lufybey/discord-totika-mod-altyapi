...
if(message.author.id === kullanıcı.user.id) return message.channel.send(new Discord.RichEmbed().setColor(client.randomrenk()).setAuthor(message.author.tag , message.author.avatarURL).setDescription(kullanıcı + " Kendini Jaile Atamazsın!")).then(msg => msg.delete(9000))

 let reason = args.slice(1).join(" ")
      if(!reason) return  message.channel.send(new Discord.RichEmbed().setColor(client.randomrenk()).setAuthor(message.author.tag , message.author.avatarURL).setDescription("Geçerli Bir Üye Girmelisin")).then(m => m.delete(5000));
...
const kanal = message.guild.channels.find(c => c.id == ayarlar.jlog) 
    ...
    .setColor(client.randomrenk())
        ...
        let puanlogs = message.guild.channels.find(c => c.id == ayarlar.puanlog) 
